...
use QuantaForge\Queue\Events\JobReleasedAfterException;
...
class WorkCommand extends Command
{
     ...
     *
     * @param  \QuantaForge\Queue\Worker  $worker
    ...
    public function handle()
    {
        ...
        $connection = $this->argument('connection')
                        ?: $this->quantaforge['config']['queue.default'];
     ...
     *
     * @return \QuantaForge\Queue\WorkerOptions
     */
    ...
    {
        $this->quantaforge['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->quantaforge['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->quantaforge['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->quantaforge['events']->listen(JobFailed::class, function ($event) {
    ...
    {
        $queueTimezone = $this->quantaforge['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->quantaforge['config']->get('app.timezone')) {
     ...
     *
     * @param  \QuantaForge\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->quantaforge['queue.failer']->log(
    ...
    {
        return $this->option('queue') ?: $this->quantaforge['config']->get(
    ...
    {
        return $this->option('force') ? false : $this->quantaforge->isDownForMaintenance();
    }
}
