...
use QuantaForge\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->quantaforge['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->quantaforge['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->quantaforge['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->quantaforge['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->quantaforge['queue.failer']->all())
                        ->where('queue', $queue)
                        ->pluck('id')
                        ->toArray();
    ...
    {
        $this->quantaforge['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->quantaforge->bound(Encrypter::class)) {
            $instance = unserialize($this->quantaforge->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
